...
- **The in-process stack is the fixture.** Dev-dependency on `iznik-testkit` gives the app a real daemon over a `unix:` alias: attach, type, flood, drop, resume — headless, bounded, in-process.
- **Claims.** Application tasks declare claims proven by tests, each with a `because`; display-bound rendering proofs say `platform`-style why they are deferred, following `darwin-artifacts`.
