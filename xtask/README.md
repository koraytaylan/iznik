...
| `distribution::windows` | The Windows server, built on Windows. Cargo writes `iznik-server.exe`; the distribution directory still carries `iznik-server`, and the upload renames it on the host. | `windows-host` |
...
| `soak::judge` | What decides whether a soak passed: everything it can refuse, in the order it refuses it, with the reason beside it. | `soak-and-release` (plan 0006) |
