//! The `iznik-regression` binary: a dispatcher that looks at the first argument only and hands the whole command line to the module that owns the subcommand.
...
use iznik_regression::{USAGE_EXIT_CODE, step};

/// The subcommands this binary routes, in the order `--help` lists them.
const SUBCOMMANDS: &[&str] = &["step"];
...
fn usage_line() -> String {
    format!(
        "usage: iznik-regression <{}> [arguments]",
        SUBCOMMANDS.join(" | ")
    )
}
