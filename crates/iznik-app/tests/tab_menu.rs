//! A tab's right click opens its menu, and the menu stays open across the
...
/// A right click on a tab opens its menu, and the menu is still there after
/// the window's own timed repaints.
#[gpui_kit::test]
fn a_tab_right_click_opens_a_menu_that_survives_repaints(context: &mut TestAppContext) {
...
/// A shell over one settled tab, right-clicked, then redrawn.
///
/// # Errors
...
/// # Panics
///
/// Panics when the tab chip is not drawn.
fn opened(context: &mut TestAppContext) -> Result<(), Failed> {
    context.update(gpui_kit::init);
    let directory: PathBuf =
        std::env::temp_dir().join(format!("iznik-tab-menu-{}", std::process::id()));
    ...
    context.update_window(handle.into(), |_, window, application| {
        assert!(window.try_find("tab-build-3").is_some(), "the tab is drawn");
        window.right_click("tab-build-3", application);
    })?;
    ...
    for _frame in 0..REPAINTS {
        context.update_window(handle.into(), |_, window, application| {
            ...
            assert!(
                window.try_find("tab-menu").is_some(),
...
/// Give the shell a connected host whose model holds one tab.
///
/// # Errors
